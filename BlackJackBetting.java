...
public class BlackJackBetting
{
	...
	private void printPlayerCards(int numCards)
	{
	...
	private void printDealerCards(int numCards)
	{
	...
	{		
		//Check the hands to see who the winner is, if they push, 
		//or if the play should continue
		...
		/*
		If there wasn't automatic BJ and dealer or player didn't win then 
		  check if they pushed. If they did win then ask player if they want 
		  to play again or quit. If none of the above then keep playing.
		*/
		if (dealerTotal() >= 17 && playerTotal() == dealerTotal() && hiddenCard == false)	
		{
	...
	private void automaticBJ()
	{
		//Checks to see if dealer has automatic BJ and then if player has auto BJ
	...
	{
		//Checks if the player wins and if not return false.
	...
	{
		/*
		Sets up the initial hands and checks asks if player wants insurance if
		Ace is showing. If not, then it checks if the dealer or player get an 
		automatic Blackjack. 
		*/
		
		if(getCardsLeft() > 10)	
		{
			//If deck has at least 10 cards continue else shuffle
			System.out.println("\nYour hand:		Dealer's Hand:");	//Set up display labels of hands
			...
			{
				//Show that the dealer's second card is hidden
				System.out.println("*Hidden*");	
	...
	public void userOption()
	{
		...
		{	
			initialOption(); //Includes option to double down 
		}
	...
	private void userLimitedOption()
	{
